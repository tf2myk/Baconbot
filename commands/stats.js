...
module.exports = {
	...
	execute(message, args) {
        ...
        if (!args.length  ) {
            return message.channel.send(`\`\`\`Use !stats (alias) to look up pug stats\n Ex: !rank myk\`\`\``);
        }
              ...
              if(!result)
              {
                ...
                for(var k = 0; k < totalcount; k++)
                {
                    ...
                    if(result[k].Alias == name)
                    {
                        ...
                        if(result[k].kills > 0)
                        {
                            kpg = result[k].kills / (result[k].Pugs - result[k].medpugs)
                            kpg = kpg.toPrecision(3)
                        }

                        if(result[k].deaths > 0)
                        {
                            dpg = result[k].deaths / (result[k].Pugs - result[k].medpugs)
                            dpg = dpg.toPrecision(3)
                        }

                        if(result[k].dpm > 0)
                        {
                            dpm = result[k].dpm / (result[k].Pugs - result[k].medpugs)
                            dpm = dpm.toPrecision(3)
                        }
